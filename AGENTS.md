...
## Current Priorities
1. Этап 5 — авто-подхват руд из других модов (ТЗ п.3).
2. Этап 6 — телепорт-блок.
3. Этап 7 — интеграция с DimWorldBorder.
Полный порядок — в `docs/ROADMAP.md`.

## Important Decisions
См. `docs/DECISIONS.md` (не переделывать без согласования):
- Генерация измерений через датапаки, а не Java-код. Единственное
  исключение — Этап 5 (сканирование реестра блоков чужих модов
  невозможно через датапак).
- Собственные модовые руды НЕ добавлять (отменено). НО руды ДРУГИХ модов
  должны авто-подхватываться в Mining World (Этап 5, ТЗ п.3).
