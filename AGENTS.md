...
6. **ОТЧЁТ ПОЛЬЗОВАТЕЛЮ — ОБЫЧНЫМИ СЛОВАМИ.** После любого обновления,
   фикса или изменения сообщать пользователю по-русски, ПРОСТЫМИ
   человеческими словами, ЧТО изменилось и ЗАЧЕМ. Без имён файлов, без
   путей, без технических терминов и без списков файлов. Максимум пара
   коротких предложений: что сделал, что это даёт, как проверить. Если
   пользователь попросил конкретно что-то — сначала сказать, что это
   сделано.
...
## Current Priorities
1. Live-проверка Этапов 4–11 (пол, телепорт-блок, данжи, динамические
   руды, предгенерация, WorldEdit, конфиг, мобы и атмосфера, полировка).
2. По итогам live-теста — подстройка плотностей/частот и фиксы.
Все этапы ROADMAP закрыты (см. `docs/ROADMAP.md`).
