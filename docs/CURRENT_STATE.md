...
Updated: 2026-08-14
Версия: 0.4.0-stage8 | Minecraft 1.20.1 | Forge 47.2.0 | Java 17

## Current Task
**Этап 8 — совместимость с WorldEdit** реализован (необязательная
зависимость; само измерение совместимо «из коробки»). Следующий по плану —
**Этап 9: конфигурация (TOML)**. Сначала — live-проверка Этапов 4–8.
...
Этап 8 (WorldEdit-совместимость) реализован, **жду live-теста**. Этапы 4–7
...
6. Нижний бедрок виден голым — фикс в Этапе 11.
...
## Next Steps
1. Проверить Этап 8 в игре: установить WorldEdit, зайти в Mining World,
   выделить область (//wand), `//set stone`, `//replace stone air` — должно
   работать как в обычном мире.
2. Проверить Этап 7 в игре: установить DimWorldBorder, задать границу мира
...
3. Проверить Этап 6 в игре: `/give @s moggminingworld:mining_portal`,
   поставить в Overworld, ПКМ → вход в Mining World, там ПКМ → выход.
4. Проверить Этапы 4–5 в игре (см. «Как проверить» ниже), удалить старые
   чанки измерения, подправить плотности/тиры.
5. Этап 9 — конфигурация (TOML).
6. Этап 11 — скрыть нижний бедрок (слой камня над ним + рваный край).

## How to Verify (Этапы 4–8)
...
9. Войти в Mining World: `/execute in moggminingworld:mining_world run tp @s 0 60 0`.
10. Ожидать (ПОСЛЕ фикса поверхности): полностью закрытое измерение —
...
11. Вернуться: `/execute in minecraft:overworld run tp @s 0 100 0`.
12. В логе сервера при генерации первых чанков будет `[Mogg] Failed to scan
...
- НЕ начинать Этап 9 до live-проверки Этапов 4–8.
