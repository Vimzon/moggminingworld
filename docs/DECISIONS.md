...
## 2026-08-14 — Этап 11: скрыть нижний бедрок ✅
Decision: плоскую `fill_layer`-фичу `bottom_bedrock` (ровный голый слой на
Y=0) УДАЛИТЬ полностью; рваный бедрок-пол оставить через `surface_rule`
(`bedrock_floor`, `above_bottom` 0..5, случайный градиент), а в
`final_density` добавить внешний `max` с `y_clamped_gradient` (+10 на Y=0
→ −10 на Y=20), чтобы нижние ~10 блоков всегда были камнем и пещеры не
прорезались до бедрока. Итог — как в Overworld: бедрок скрыт толщей породы.
Reason: live-тест показал «голый ровный bedrock внизу» — виновником была
ровная fill_layer-фича, а не surface_rule. Поднимать min_y/пол мир (другие
варианты из ROADMAP) не нужно — проще и безопаснее поднять плотность
камня над бедроком. [Фикс 2026-08-14: изначально был `min` с градиентом
−10..+10 — это НЕВЕРНО: в noise `final_density` блок ставится при значении
> 0, а `min` с отрицательным градиентом на дне даёт воздух (пустота вместо
пола). Заменено на `max` с градиентом +10..−10: `max` гарантирует твёрдость
там, где градиент положительный, и не мешает пещерам выше Y≈10.]
Do not: НЕ возвращать `bottom_bedrock` (fill_layer); НЕ опускать min_y/пол;
НЕ менять кэши/рудные диапазоны под новую высоту пола (камень начинается
с Y=0, просто пещеры ниже ~Y=10 не режутся); НЕ использовать `min` с
отрицательным градиентом для «заливки» пола — только `max` с положительным.

## 2026-08-14 — Фиксы по адверсариальному ревью (портал + руды с мобов) ✅
Decision: (1) `MiningPortalBlock.findSafeSpawn` — вместо поиска точки
только вниз от Y=100 (и опасного фолбэка ВНУТРЬ блока → удушение) сканирует
всю колонну сверху вниз, а если колонна сплошная — спиралью наружу радиусом
до 32 блоков; точка всегда «2 воздуха + твёрдый блок под ногами». (2)
`OreDropModifier` — условие `hasParam(THIS_ENTITY)` заменено на
`instanceof Mob`: раньше GLM срабатывал и на дропе ЛЮБОГО блока в Mining
World (в блочном луте THIS_ENTITY = игрок), т.е. руды сыпались и при ломании
камня. Теперь — только с убийств мобов, как задумано.
Reason: оба бага найдены адверсариальным ревью: вход в измерение мог
задушить игрока, а «руды с мобов» фактически работали как «руды с любого
блока», разгоняя экономику.
Do not: НЕ возвращать фолбэк телепорта в координату без проверки воздуха;
НЕ менять условие GLM обратно на `hasParam` без проверки типа сущности
(игрок — тоже THIS_ENTITY в блочном луте).
