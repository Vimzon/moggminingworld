...
## 2026-08-14 — Этап 10: мобы и атмосфера ✅
Decision: без собственного мода-моба — в биом добавлены ВАНИЛЬНЫЕ
пещерные мобы (зомби/скелет/пауки/крипер/эндермен + летучие мыши), а
«руды с мобов» реализованы через Forge Global Loot Modifier
(`loot/OreDropModifier.java` + датапаки `loot_modifiers/`). Шанс — из
конфига, пул руд — из датапака. Атмосфера: туман глубины через
`ViewportEvent` (клиент) и `fixed_time` в dimension_type (нет дня/ночи).
Reason: кастомный моб (entity+renderer+texture+spawn egg) — большой объём
и риск без live-теста; ванильные мобы + GLM дают требуемое «мобы с дропом
руд» надёжно. GLM — штатный Forge-механизм, не ломает Overworld.
Do not: НЕ добавлять кастомную сущность моба без явной просьбы; НЕ менять
ванильный дроп за пределами Mining World.

## 2026-08-14 — Этап 9: конфигурация TOML ✅
Decision: все настраиваемые параметры (тиры/частоты/размеры динамических
руд Этапа 5, скорость предгенерации, правило «ограничение копания вверх»)
вынесены в ForgeConfigSpec (`MiningWorldConfig`) → автогенерация
`moggminingworld-common.toml`. Захардкоженные значения `OreTier` enum в
`DynamicOreFeature` убраны, значения читаются из конфига.
Reason: параметры генерации меняются в ходе live-тестов — конфиг позволяет
подстраивать без пересборки/перекомпиляции.
Do not: НЕ хардкодить новые настраиваемые параметры; НЕ подключать
дополнительный конфиг-фреймворк — ForgeConfigSpec достаточно.

## 2026-08-14 — Этап 8: WorldEdit-совместимость без кода генерации ✅
Decision: никакой Java-интеграции с WorldEdit не требуется — для Forge он
работает с любым загруженным `ServerLevel`, а Mining World — стандартный
`minecraft:noise`-генератор (min_y 0, height 320). В `mods.toml` добавлена
необязательная зависимость `worldedit`. Проверка — только в игре
(`//wand`, `//set`, `//replace`).
Reason: измерение — обычный ServerLevel; специальный код не нужен и лишь
добавил бы точку отказа.
Do not: не писать собственную интеграцию/обработку команд WorldEdit;
не делать зависимость обязательной.

## 2026-08-14 — Этап 7: предгенерация чанков + DimWorldBorder ✅
Decision: DimWorldBorder работает с любым измерением автоматически
(навешивает слушатели границы на все измерения на старте сервера и
применяет сохранённые настройки), поэтому правок его API не требуется.
Наш вклад — команда `/moggminingworld pregen start [radius]` (stop/status),
которая генерирует чанки мира майнинга порциями (8/тик) вокруг центра
границы: без радиуса берётся текущая граница мира (задаётся через
`/dimworldborder moggminingworld:mining_world set ...`), с радиусом —
квадрат вокруг центра границы. Добавлена НЕобязательная зависимость
`dimensionalworldborder` в mods.toml.
Reason: пользователь хочет ограничить мир майнинга границей и иметь
полностью сгенерированную зону внутри неё.
Do not: не делать предгенерацию синхронной (фризит сервер); не хардкодить
размер границы в коде (всегда читать WorldBorder); не делать зависимость
от DimWorldBorder обязательной.

## 2026-08-14 — Телепорт-блок — админ-блок без крафта (Этап 6) ✅
Decision: вход/выход в Mining World — через единый блок `mining_portal`
(ПКМ). Блок админский: strength `-1`/`3600000` (неломаемый в выживании),
без лут-таблицы, без крафта/рецепта — выдаётся `/give @s
moggminingworld:mining_portal` (или админ-панель). В Overworld: ПКМ →
телепорт в Mining World; в Mining World: ПКМ → возврат в Overworld. Точка
входа — `0,100,0` каждой измерики, с поиском безопасной позиции
(свободный блок над твёрдым) вниз по колонне.
Reason: ТЗ — «портал в Overworld, обратная телепортация»; никаких команд
варпа. Админ-блок — простота и контроль места входа (ставится один раз на
нужной точке).
Do not: не добавлять рецепт крафта портала; не добавлять команды
возврата/варпа; не делать телепортацию по клику в воздухе (только ПКМ по
блоку).
...
## Пещеры — ванильная схема OVERWORLD вместо Незер-залов (фикс 2026-08-14)
...
## Руды — высотное распределение как в верхнем мире (фикс 2026-08-14)
- **Проблема (live-feedback)**: iron/copper были только внизу (0..80/0..112),
  gold ОШИБОЧНО 80..320 (только вверху) — на большой высоте были только
  уголь/изумруд/золото. Руды модов — только до Y 120, жилы 5..10.
- **Решение**: все базовые руды — по всей колонне (0..320), ценные
  (gold/lapis/redstone/diamond) сдвинуты вниз (0..128/0..64/0..32),
  diamond жирнее (count 4). Руды модов: тиры COMMON/MID/DEEP/BOTTOM
  = 0..320/0..256/0..192/0..128, size 14/12/10/8, height_range 0..320.
- **Do not**: не возвращать узкие диапазоны только «внизу» для железа/меди;
  не ставить золото в верхние слои.